...
- **【本轮最高价值·已确认】项目已发布的实股头部数字在本机完全复现。** 我在同一进程内 import 仓库自己的 `src/ml/final_holdout.py`，把模块级 `REPORT_DIR` 重定向到 scratch 后调用 `main()`（已跟踪 JSON 运行前备份、运行后确认 `unchanged=True`，未覆盖任何产物）。**91.4s** 重跑得到与 `reports/ml_final_holdout.json` **逐位相同**的 30 个字段：`n_train_rows=2,140,262` / `n_holdout_rows=476,860` / `n_purged_rows=31,663` / `base_rate=0.028958184792182193` / `threshold=0.16580970336242568` / `signals=6,202` / `hits=844` / `precision=0.1360851338277975` / `lift=4.6993668561896955` / `distinct_stocks=1,194` / `date_hhi=0.026392023211661234` / `busiest_date=2026-07-20=421`。即：**2,648,785 个成熟行的严格时间外分割、purge 只在 session 空间做（`final_holdout.py:135 purge_by_label_end`，`:147` 明写 "market sessions, not rows"）、base 2.8958%、6202 信号 844 命中、13.61%、lift 4.70x 是真实且可复现的 A 股结果。** 因此长期沿用的「本项目没有真实市场证据」框架**半边错误**：准确说法是「已有可复现的真实 OOS 数字，但没有任何自动化装置在读它、也没有任何审计在重跑它」。
- **【本轮新发现·已确认】重跑暴露一个真实的陈旧不确定性区间。** 已跟踪 JSON 缺当前代码会写的 3 个字段，且其 `date_clustered_95` 等于当前代码的 **block1 变体**：tracked `[0.10238,0.17587]`（宽 0.07349）vs **当前定义** `[0.08599,0.20896]`（宽 0.12297，**+67.3%**，即已发布区间**低估不确定性**）。成因：`c584ffd`(2026-09-18 13:26:35) 改了 `date_clustered_ci`（提交信息自述旧法 "understating the published interval"），**只改代码未重跑 JSON**；其后 `86c2e25` 又改代码但 JSON 只补 note/exposure 文本字段，从未重生成 CI。`reports/AUDIT_STATUS.md` 发布的就是这个旧窄区间。修法：重跑 `python -m src.ml.final_holdout`（~91s）；`audit_reports.py` 应断言 JSON 键集合完整。**注意区分**：头部点估计完全复现，只有**区间**陈旧。
- **【第三方可复现性边界】** 断言被跟踪（`reports/*.json`），所依赖的数据被忽略（`outputs/ml/**/*.parquet` `.gitignore:29`、`data/*.parquet` `:2`、`outputs/*_signals*.csv` `:21`）。故**本机可复现、克隆者不可复现**。`reports/ml_precision_ceiling.json`（`oos_rows=1,406,181`/`oos_positives=57,505`）我也**未能**从已跟踪的 `real_oos_scores.npz`（281,227 行/462 dates/2024-02..2025-12，是另一个实验）复现 → `未复现（缺未跟踪输入）`，与"数字错误"是两回事。
- **【未复现·待验证风险】** `2026-09-21_14-06-49_JST.md` 的 `33 passed in 5.02s`、4个候选SHA-256、v3→v4 patch，在本仓库**无对应实体**：全部可收集测试实测仅 **15**（`pytest --collect-only` = 15；`tests/` 是唯一测试根；无 `pyproject.toml`/`pytest.ini`/`setup.cfg`）；tracked `*.zip/*.patch/*.pt/*.jsonl/*.receipt` 全为 0；`src/ml/research_h504/` 不存在；4个哈希只出现在2个 `.md` 散文中。**不指控伪造**（候选可能在审计沙箱），但一律记 `未复现`，不得当作本项目已验收证据。两个独立只读子代理各自独立复核了同一结论（其中一个扫描了 121,760 个磁盘文件 + 全部 3,263 个 git 对象，0 命中）。
